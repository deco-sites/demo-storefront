...
import { withoutPoweredBy } from "./server/withoutPoweredBy";
...
// `withoutPoweredBy` (see ./server/withoutPoweredBy.ts) sits inside
// `instrumentWorker` (which adds no response headers of its own) so the OTel
// wrapper stays the outermost layer. Its regression test lives next to it.
//
